...
6. **Compiler Integration vs. Cleartext Helpers**: 
   Every FHE-compatible function MUST be compiled and tested using `fhe.Compiler` (Rule 13). However, client-side preprocessing or helper functions that inherently cannot operate on encrypted `Tracer` variables (e.g., `encode_fixed_point`, `unsigned_to_bits`) are EXEMPT from compiler integration. These specific helpers must be tested in standard cleartext-only mode, and their markdown block must explicitly state that they are client-side helpers not meant for FHE compilation.
